...
function obtenerDatosCurso(curso) {
  var textoIntroduccion = mensajes.texto_introduccion
    .replace("X1", curso.id)
    .replace("X2", curso.nombre);
...
function guardarRegistro(
  ...
  datos_entrada,
  nombre_archivo,
  mensaje_exito_registro
) {
  var info_matricula =
    mensajes.texto_matricula
      .replace("X1", datos_entrada.n)
      .replace("X2", datos_entrada.x) + obtenerDatosCurso(dato_curso);
  fs.writeFile(nombre_archivo, info_matricula, err => {
    if (err) throw err;
    mostrarPorConsola(mensaje_exito_registro);
  });
}

function mostrarEnNavegador(
  dato_curso,
  datos_entrada,
  nombre_archivo,
  mensaje_exito_registro
  ...
  app.get("/", function(req, res) {
    res.render("index", {
      ...
      message: info_matricula,
      messageInto: "Registro exitoso"
    });
  });
  app.listen(3000);
}
...
function inicio(cursos, argv, nombre_archivo, mensaje_exito_registro) {
  if (validarExistenciaComando(argv)) {
    ...
    if (validarExistenciaCurso(curso)) {
      mostrarPorConsola(
        "Por favor dirijase al navegador con diección localhost puerto 3000 y vea el mensaje , ej: http://localhost:3000"
      );
      mostrarEnNavegador(curso, argv, nombre_archivo, mensaje_exito_registro);
    } else {
